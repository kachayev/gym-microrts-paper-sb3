...
class MicroRTSStatsRecorder(VecEnvWrapper):
    ...
    def step_wait(self):
        ...
        for ind in range(len(dones)):
            self.raw_rewards[ind].append(infos[ind]["raw_rewards"])
        newinfos = list(infos[:])
        raw_names = [str(rf) for rf in self.rfs]
        for ind, done in enumerate(dones):
            if done:
                info = infos[ind].copy()
                raw_rewards = np.array(self.raw_rewards[ind]).sum(0)
                info["microrts_stats"] = dict(zip(raw_names, raw_rewards))
                self.raw_rewards[ind] = []
                newinfos[ind] = info
...
class MicroRTSExtractor(nn.Module):
    ...
    def forward(self, features):
        obs, masks = features
        shared_latent = self.latent_net(obs)
        return self._mask_action_logits(self.policy_net(shared_latent), masks), self.value_net(shared_latent)

    def forward_actor(self, features):
        obs, masks = features
        return self._mask_action_logits(self.policy_net(self.latent_net(obs)), masks)
...
class MicroRTSGridActorCritic(ActorCriticPolicy):
    ...
    def __init__(self, observation_space, action_space, *args, **kwargs):
        ...
        if self.mlp_extractor.latent_dim_vf == 1:
            self.value_net = nn.Identity()
